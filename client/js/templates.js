  ...
  'data-table': {
    ...
    init: () => {
      ...
      window.sortDemoTable = () => {
        ...
        window.renderDemoTable(sorted);
      };

      window.renderDemoTable(data);
    },
  },
};
...
function buildCodePreviewDemo(template) {
  const codeSnippet = (template.code || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
  const tags = (template.tags || [])
    .map(
      (t) =>
        '<span style="display:inline-block;padding:2px 8px;border-radius:var(--radius-sm);font-size:0.7rem;font-weight:600;background:rgba(61,122,95,0.2);color:var(--forest-green-accent);border:1px solid rgba(61,122,95,0.3);">' +
        t +
        '</span>'
    )
    .join('');
...
demoConfigs['rbac-middleware'] = {
  ...
  init: () => {
    ...
    function getEffective(r) {
      return [r, ...(HIERARCHY[r] || [])];
    }
    function check(role, perm) {
      return (PERMS[perm] || []).some((r) => getEffective(role).includes(r));
    }
...
demoConfigs['chain-hashed-audit-logger'] = {
  ...
  init: () => {
    ...
    document.getElementById('auditTamperBtn').addEventListener('click', () => {
      if (entries.length < 2) {
        alert('Log at least 2 entries first');
        return;
      }
  ...
  init: () => {
    function simEncrypt(text) {
      ...
      for (let i = 0; i < text.length; i++) {
        enc += (text.charCodeAt(i) ^ 0x5a).toString(16).padStart(2, '0');
      }
...
demoConfigs['pii-data-masking'] = {
  ...
  init: () => {
    document.getElementById('maskBtn').addEventListener('click', () => {
      ...
      text = text.replace(/\b(\d{3})[-.\s]?(\d{3})[-.\s]?(\d{4})\b/g, (m, a, b, c) => {
        count++;
        return '(***) ***-' + c;
      });
      text = text.replace(/\b(\d{4})[- ]?(\d{4})[- ]?(\d{4})[- ]?(\d{4})\b/g, (m, a, b, c, d) => {
        count++;
        return '**** **** **** ' + d;
      });
...
demoConfigs['breach-notification-engine'] = {
  ...
  init: () => {
    const FW = {
      dora: { auth: 'National Competent Authority', hours: 4, label: 'DORA Initial' },
      nis2: { auth: 'National CSIRT', hours: 24, label: 'NIS 2 Early Warning' },
      gdpr: { auth: 'Supervisory Authority (DPA)', hours: 72, label: 'GDPR Art. 33' },
      hipaa: { auth: 'HHS OCR', hours: 1440, label: 'HIPAA Breach Notification' },
    ...
    document.getElementById('breachReportBtn').addEventListener('click', () => {
      ...
      const frameworks = [...document.querySelectorAll('#breachFrameworks input:checked')].map(
        (i) => i.value
      );
      let severity = 'LOW';
      if (count > 500) {
      ...
      if (['phi', 'credentials'].includes(dataType) && count > 500) {
        severity = 'CRITICAL';
      }
      if (['phi', 'pii', 'financial', 'credentials'].includes(dataType)) {
      ...
      let html =
        '<div style="color:' +
        colors[severity] +
        ';font-weight:700;font-size:1.1rem;margin-bottom:var(--spacing-sm);">⚠ Severity: ' +
        severity +
        '</div>';
      html +=
        '<div style="color:var(--text-secondary);margin-bottom:var(--spacing-md);font-size:0.85rem;">Incident ID: INC-' +
        Date.now() +
        ' · ' +
      ...
      if (frameworks.length === 0) {
        html += '<div style="color:var(--text-muted);">No frameworks selected.</div>';
      } else {
        html +=
          '<div style="font-weight:600;color:var(--text-primary);margin-bottom:var(--spacing-xs);">📋 Notification Deadlines (sorted by urgency):</div>';
        const deadlines = frameworks
          .map((fw) => {
            const d = FW[fw];
            const dl = new Date(now.getTime() + d.hours * 3600000);
            return { ...d, fw, deadline: dl };
          })
        ...
        deadlines.forEach((d) => {
          ...
          html +=
            '<div style="padding:var(--spacing-sm);margin-bottom:4px;background:' +
            (urgent ? 'rgba(239,68,68,0.1)' : 'var(--bg-tertiary)') +
          ...
          html +=
            '<span style="color:' +
            (urgent ? '#ef4444' : 'var(--text-muted)') +
            ';">⏰ ' +
            remaining +
            'h remaining - Deadline: ' +
  ...
  init: () => {
    ...
    document.getElementById('mfaVerifyBtn').addEventListener('click', () => {
      ...
      document.getElementById('mfaAttempts').textContent =
        'Attempts: ' + attempts + '/5' + (locked ? ' - LOCKED' : '');
    });
  },
};
  ...
  init: () => {
    const profiles = {
      standard: {
        ...
        label: 'Standard',
        color: '#10b981',
        use: 'General web applications, internal tools',
      },
      sensitive: {
        idle: 15,
        absolute: 240,
        ...
        label: 'Sensitive',
        color: '#f59e0b',
        use: 'PCI DSS CDE access, HIPAA ePHI systems',
      },
      critical: {
        idle: 5,
        absolute: 60,
        ...
        label: 'Critical',
        color: '#ef4444',
        use: 'Financial trading, military systems, root access',
      },
    };
    ...
    document.getElementById('sesStartBtn').addEventListener('click', () => {
      if (!activeProfile) {
        showProfile('standard');
      }
      const p = profiles[activeProfile];
      maxIdle = p.idle * 6;
      warnAt = (p.idle - p.warn) * 6;
      idleSeconds = 0;
...
demoConfigs['hipaa-phi-filter'] = {
  ...
  init: () => {
    const record = {
      names: 'Jane Doe',
      geographic: '1234 Elm St, Portland OR 97201',
      dates: '1985-03-15',
      phone: '503-555-0199',
      email: 'jane.doe@email.com',
      ssn: '123-45-6789',
      mrn: 'MRN-00482',
      healthPlanId: 'BCBS-9921',
      accountNumbers: 'ACCT-3847',
      diagnosis: 'Type 2 Diabetes',
      medications: 'Metformin 500mg',
    };
    const roles = {
      treating_provider: {
        access: Object.keys(record),
        label: 'Full PHI (Treatment)',
        color: '#10b981',
      },
      billing: {
        access: ['names', 'mrn', 'healthPlanId', 'accountNumbers', 'dates'],
        label: 'Billing Subset',
        color: '#f59e0b',
      },
      researcher: {
        access: ['diagnosis', 'medications'],
    ...
    document.getElementById('phiFilterBtn').addEventListener('click', () => {
      ...
      Object.entries(record).forEach(([key, val]) => {
        ...
        html +=
          '<div style="display:flex;justify-content:space-between;padding:6px var(--spacing-sm);background:' +
          (allowed ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)') +
...
demoConfigs['gdpr-consent-manager'] = {
  ...
  init: () => {
    ...
    document.getElementById('consentExportBtn').addEventListener('click', () => {
      ...
      a.download = 'gdpr-consent-export.json';
      a.click();
    });
  },
};
...
demoConfigs['pci-card-sanitizer'] = {
  ...
  init: () => {
    ...
    function detect(d) {
      return brands.find((b) => b.test(d))?.name || 'Unknown';
    }
    function mask(d) {
    ...
    function tokenize(d) {
      ...
      return 'tok_' + Math.abs(h).toString(16).padStart(16, '0').slice(0, 16);
    }
    ...
    document.getElementById('pciSanitizeBtn').addEventListener('click', () => {
      ...
      if (!valid) {
        html =
          '<div style="padding:var(--spacing-md);background:rgba(239,68,68,0.1);border-radius:var(--radius-md);color:#ef4444;font-weight:600;">✗ Invalid card number (Luhn check failed)</div>';
      } else {
        const items = [
        ...
        items.forEach((i) => {
          html +=
            '<div style="display:flex;justify-content:space-between;align-items:center;padding:var(--spacing-sm);background:var(--bg-card);border:1px solid var(--border-light);border-radius:var(--radius-md);">' +
            '<span style="color:var(--text-secondary);font-weight:600;font-size:0.85rem;">' +
            i.label +
            '</span>' +
            '<span style="font-family:var(--font-mono);font-size:0.9rem;color:' +
            i.color +
            ';font-weight:600;">' +
            i.value +
            '</span></div>';
    ...
    const perspectives = [
      {
        ...
        objectives: [
          {
            obj: 'Transition to circular, regenerative models',
            kpi: '% PaaS revenue > 35%',
            ai: 'AI dynamic pricing',
          },
          {
            obj: 'Reduce OpEx via ecological efficiency',
            kpi: '20% energy cost reduction',
            ai: 'Autonomous energy grid balancing',
          },
        ],
      },
      {
        ...
        objectives: [
          {
            obj: 'Enhance brand equity via purpose-driven impact',
            kpi: 'NPS + Trust Index > 75',
            ai: 'NLP sentiment analysis',
          },
          {
            obj: 'Ethical AI service accessibility',
            kpi: '100% bias audit pass',
            ai: 'Automated bias detection',
          },
        ],
      },
      {
        ...
        objectives: [
          {
            obj: 'Zero-waste closed-loop supply chain',
            kpi: '> 65% material recovery',
            ai: 'Computer vision reverse logistics',
          },
          {
            obj: 'Operational carbon neutrality',
            kpi: 'Net-Zero Scope 1 & 2',
      ...
      },
      {
        ...
        objectives: [
          {
            obj: 'Culture of continuous green innovation',
            kpi: '10 patents/year',
            ai: 'AI knowledge management',
          },
          {
            obj: 'Inclusive, psychologically safe environment',
            kpi: '< 5% voluntary turnover',
            ai: 'Anonymized inclusion analysis',
          },
        ],
      },
    ];
    ...
    const phases = [
      {
        ...
        ai: 'AI audits supplier ESG reports',
        kpi: '% circular procurement spend',
      },
      {
        name: 'Circular Design',
        icon: '🔩',
        action: 'Modularity, durability, repairability by design',
        ai: 'Generative AI material simulation',
        kpi: 'Product Circularity Index',
      },
      {
        name: 'GreenOps Manufacturing',
        icon: '🏭',
        action: 'Zero-waste, renewable-powered production',
        ai: 'Digital twins optimize energy & waste',
        kpi: 'Waste diverted from landfill',
      },
      {
        name: 'PaaS Distribution',
        icon: '📦',
        action: 'Leasing & subscription over transactional sales',
        ai: 'AI demand forecasting',
        kpi: 'Asset utilization rate',
      },
      {
        name: 'Reverse Logistics',
        icon: '🔄',
        action: 'Collection hubs & incentivized take-back',
        ai: 'Intelligent multi-stop routing',
        kpi: 'Products recovered (volume)',
      },
      {
        name: 'Remanufacturing',
        icon: '♻️',
        action: 'Sort, disassemble, triage for reuse/recycle',
        ai: 'Computer vision component classification',
    ...
    const phases = [
      {
        ...
        criteria: 'Campaign reflects verified operational capabilities?',
      },
      {
        name: 'Impact Verification & Evidence',
        criteria: 'All ESG claims substantiated by audit-quality data?',
      },
      {
    ...
    const phases = [
      {
        name: 'Requirements',
        icon: '📋',
        action: 'Define scope with sustainability focus',
        ai: 'AI predicts compute needs',
        green: 'Cloud lifecycle costs & ESG alignment',
      },
      {
        name: 'Architecture',
        icon: '🏗️',
        action: 'Cloud-native, modular design',
        ai: 'AI assists energy-efficient schema',
        green: 'Reduced duplication & data center efficiency',
      },
      {
        name: 'Development',
        icon: '💻',
        action: 'Clean code, efficient algorithms',
        ai: 'Copilots optimize for lower CPU/memory',
        green: 'Code complexity & debt reduction',
      },
      {
        name: 'Testing',
        icon: '🧪',
        action: 'Functional + security + environmental',
        ai: 'ML CI/CD pipelines reduce QA hours',
        green: 'Server load optimization',
      },
      {
        name: 'Deployment',
        icon: '🚀',
        action: 'Continuous monitoring & observability',
        ai: 'AI detects energy anomalies; auto-scale',
    ...
    const funcs = [
      {
        ...
        icon: '🏛️',
        color: '#d4a574',
        action: 'Cross-functional oversight; AI policies & risk tolerances',
        ai: 'AI aggregates regulatory updates (EU AI Act, CSRD)',
        docs: 'AI Charters → ISO 42001',
      },
      {
        name: 'MAP',
        icon: '🗺️',
        color: '#7ec99b',
        action: 'Document all AI systems, data dependencies, impacts',
        ai: 'Asset discovery scans data flows & shadow AI',
        docs: 'Architecture Diagrams → GDPR/SOC 2',
      },
      {
        name: 'MEASURE',
        icon: '📏',
        color: '#5a9d7a',
        action: 'Assess bias, hallucination, security, energy costs',
        ai: 'Automated Red Teaming & adversarial testing',
        docs: 'Bias Reports → OSCAL evidence',
      },
      {
        name: 'MANAGE',
        icon: '🔧',
        color: '#9dbd7e',
        action: 'Mitigate risks; continuous monitoring; human-in-the-loop',
        ai: 'Real-time observability; auto-alerts on anomalies',
  ...
  init: () => {
    ...
    window.executeApiCall = () => {
      ...
      if (method === 'GET-all') {
        ...
        response = {
          status: 200,
          data: { id, name, updated_at: new Date().toISOString() },
    ...
    window.apiClientError = async () => {
      ...
      try {
        ...
        showResponse({
          error: err.message,
          handled: true,
          suggestion: 'Implement retry logic or fallback',
        });
      }
    };
  },
  ...
  init: () => {
    ...
    window.previewEmailTemplate = () => {
      ...
      try {
        ...
        vars = { name: 'User', company: 'Company' };
      }
  ...
  init: () => {
    ...
    window.handleThrottleMove = (e) => {
      ...
      if (canThrottle) {
        ...
        document.getElementById('throttleArea').textContent =
    ...
    function updateEfficiency() {
      const debounceEff =
        debounceKeystrokes > 0 ? Math.round((1 - debounceCalls / debounceKeystrokes) * 100) : 0;
      const throttleEff =
