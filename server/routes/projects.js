...
router.post('/',
    ...
    (req, res) => {
        ...
        const { title, client_id, description, status, priority, start_date, end_date, budget, hours_estimated } = req.body;
        ...
        try {
            ...
            logActivity(req.user.userId, 'CREATE', 'project', result.lastInsertRowid, `Created project: ${title} for ${client.name}`);
...
router.put('/:id',
    ...
    (req, res) => {
        ...
        try {
            ...
            const { title, description, status, priority, start_date, end_date, budget, hours_estimated, hours_actual, completion_date } = req.body;
            ...
            if (hours_actual !== undefined) { updates.push('hours_actual = ?'); params.push(hours_actual); }
            ...
            logActivity(req.user.userId, 'UPDATE', 'project', id, `Updated project: ${updatedProject.title}`);
...
router.post('/:projectId/tasks',
    ...
    (req, res) => {
        ...
        const { title, description, status, priority, hours_estimated, due_date, assigned_to } = req.body;
        ...
        try {
            ...
            const result = execute(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `, [projectId, title, description || null, status || 'pending', priority || 'medium', hours_estimated || null, due_date || null, assigned_to || null]);
...
router.put('/:projectId/tasks/:taskId', (req, res) => {
    ...
    const { title, description, status, priority, hours_estimated, hours_actual, due_date, assigned_to } = req.body;
    ...
    (req, res) => {
        ...
        try {
            ...
            const result = execute(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `, [projectId, task_id || null, req.user.userId, hours, description || null, entry_date, billable !== false ? 1 : 0]);
            
            // Update project hours_actual
            const totalHours = queryOne('SELECT SUM(hours) as total FROM time_entries WHERE project_id = ?', [projectId])?.total || 0;
            execute('UPDATE projects SET hours_actual = ? WHERE id = ?', [totalHours, projectId]);
            
            const newEntry = queryOne('SELECT * FROM time_entries WHERE id = ?', [result.lastInsertRowid]);
