...
router.post(
  ...
  [
    ...
    body('client_ids.*').optional().isInt().withMessage('Each client_id must be an integer'),
  ],
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
// Delete campaign
router.delete('/:id', requireOwnership('campaign', 'id'), (req, res) => {
  const { id } = req.params;
...
router.post(
  ...
  (req, res) => {
    ...
    try {
      ...
      const template = queryOne('SELECT * FROM email_templates WHERE id = ?', [
        result.lastInsertRowid,
      ]);
