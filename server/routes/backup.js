...
function sanitizeFilename(value) {
  if (/[/\\]/.test(value) || value.includes('..')) {
    throw new Error('Invalid filename — path separators and ".." are not allowed');
  }
  return value;
}
...
router.post(
  ...
  [
    ...
    body('customPath').optional().isString().custom(sanitizeFilename).withMessage('Invalid path'),
  ],
  (req, res) => {
...
router.get(
  ...
  authenticateToken,
  requireRole('admin'),
  [param('filename').isString().custom(sanitizeFilename)],
  (req, res) => {
    const valErr = handleValidation(req, res);
    if (valErr) {
      return;
...
router.post(
  ...
  requireRole('admin'),
  [
    body('filename').optional().isString().custom(sanitizeFilename).withMessage('Invalid filename'),
  ...
  (req, res) => {
    ...
    try {
      ...
      logActivity(
        ...
        'backup',
        null,
        `Restored from: ${filename || 'uploaded data'}`
      );
      ...
      res
        .status(500)
        .json({ success: false, message: 'Failed to restore database: ' + error.message });
    }
  }
);
