...
export async function initializeDatabase() {
  ...
  // P3.2 — Add MFA columns to users table (safe for existing databases)
  const userColumns = db
    .exec('PRAGMA table_info(users)')
    .map((r) => r.values.map((v) => v[1]))
    .flat();
  if (!userColumns.includes('mfa_enabled')) {
    db.run('ALTER TABLE users ADD COLUMN mfa_enabled INTEGER DEFAULT 0');
  }
  if (!userColumns.includes('mfa_secret')) {
    db.run('ALTER TABLE users ADD COLUMN mfa_secret TEXT');
  }
  if (!userColumns.includes('mfa_temp_secret')) {
    db.run('ALTER TABLE users ADD COLUMN mfa_temp_secret TEXT');
  }

  // P3.2.3 — Refresh token table for session management
  db.run(`
    ...
    `);
  db.run('CREATE INDEX IF NOT EXISTS idx_refresh_token_hash ON refresh_tokens(token_hash)');
  db.run('CREATE INDEX IF NOT EXISTS idx_refresh_token_family ON refresh_tokens(family)');
...
async function seedTestData() {
  ...
  if (userCount === 0) {
    ...
    const existingAdmin = query(`SELECT id FROM users WHERE username = ? OR email = ?`, [
      ADMIN_USERNAME,
      ADMIN_EMAIL,
    ]);
